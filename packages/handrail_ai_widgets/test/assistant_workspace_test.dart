  ...
  HandrailWorkspaceBinding get binding => (
        ...
        transcriberFor: (_) => ({
              ...
              required cancellation,
                ...
                (text: 'dictated words', errorCode: null, retryable: false),
        send: (
...
void main() {
  ...
  testWidgets(
    ...
    await tester.enterText(
        find.byKey(const ValueKey('draft')), '123456789012345');
    await tester.pump();
    ...
    await tester.enterText(
        find.byKey(const ValueKey('expanded')), 'Next draft');
  ...
  testWidgets(
    ...
    expect((request['metadata'] as Map)['handrail_approval_mode'], 'required');
  ...
  testWidgets(
    ...
    fixture.drafts.addAttachments([
      HandrailAttachmentFile(
          fileName: 'photo.png', mediaType: 'image/png', bytes: [1, 2, 3]),
    ]);
    await tester.enterText(find.byKey(const ValueKey('draft')), 'First draft');
    await tester.pump();
    expect(fixture.drafts.controller.text, 'First draft',
    ...
    fixture.publish();
    await tester.pump();
    await tester.enterText(find.byKey(const ValueKey('draft')), 'Second draft');
    ...
    expect(
        fixture.requests.single.request['business_context'], 'original-route');
  ...
  testWidgets(
      ...
      (tester) async {
    final first = Fixture()..admission = Completer<bool>(), second = Fixture();
    addTearDown(first.dispose);
    addTearDown(second.dispose);
    await tester
        .pumpWidget(surface(first, mode: HandrailApprovalMode.automatic));
    await tester.pumpAndSettle();
    await tester.enterText(find.byKey(const ValueKey('draft')), 'Old account');
    await tester.pump();
    await tester.tap(find.byKey(const ValueKey('send')));
    await tester.pump();
    await tester.pumpWidget(surface(second));
    await tester.pumpAndSettle();
    await tester.enterText(find.byKey(const ValueKey('draft')), 'New account');
    ...
    testWidgets(
      ...
      expect(
          tester
              ...
              .compact,
          width < 720);
      expect(find.byKey(const ValueKey('send')).hitTestable(), findsOneWidget);
      expect(find.byKey(const ValueKey('draft')).hitTestable(), findsOneWidget);
      expect(tester.takeException(), isNull);
    });
  }
}
